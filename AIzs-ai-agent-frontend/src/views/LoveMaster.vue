...
const STORAGE_KEY_PREFIX = 'ai_love_app_chat_history_'
...
// 页面加载时添加欢迎消息
onMounted(() => {
  const savedId = localStorage.getItem('ai_love_app_current_chat_id')
  if (savedId) {
    chatId.value = savedId
    loadMessages()
    if (messages.value.length === 0) {
      addMessage('欢迎来到AI恋爱大师，请告诉我你的恋爱问题，我会尽力给予帮助和建议。', false)
    }
...
onBeforeUnmount(() => {
  ...
  localStorage.setItem('ai_love_app_current_chat_id', chatId.value)
})
</script>
